...
function slist (target) {
  ...
  items = target.getElementsByTagName("li"), current = null;
...
function checkAnswers() {
	...
	let isGood = true;
	for (let it=0; it<expectedArray.length; it++) {
		items[it].classList.remove("good");
		items[it].classList.remove("wrong");
		
		let a = expectedArray[it];
		let b = items[it].textContent;
		if (a != b && isGood) {
			isGood = false;
			console.log("Expected: "+a	+" got: "+b);
		}
	}
	items = document.getElementById("sortlist").getElementsByTagName("li");
	if (isGood) {
		for (let it=0; it<items.length; it++) {
		...
		for (let it=0; it<items.length; it++) {
			items[it].classList.add("wrong");
		}
	}
}

function show() {
	document.getElementByClass('image').style.display = "block";
	document.getElementByClass('btnID').style.display = "none";
}
